...
function getLevelColor(level: string): string {
  const colors: Record<string, string> = { 'Initial': '#ef4444', 'Repeatable': '#f97316', 'Defined': '#eab308', 'Managed': '#22c55e', 'Optimised': '#3b82f6' }; return colors[level] || '#666'
}
function getScorePosition(score: number): number {
  if (score < 2) return 0; if (score < 3) return 1; if (score < 4) return 2; if (score < 5) return 3; return 4
}
...
function GatedOverlay({ processName }: { processName: string }) {
  ...
  const handleUnlockRequest = async () => {
    ...
    if (user) {
      await supabase.from('leads').insert({ email: user.email, full_name: user.user_metadata?.full_name || '', org_name: user.user_metadata?.org_name || '', source: 'unlock_request', role_type: processName })
    }
    setSubmitting(false); setSubmitted(true)
  }
  return (
    <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', minHeight: '500px' }}>
...
function RadarChart({ results, hoveredCode, onHover, isUnlocked }: { results: L2Result[], hoveredCode: string | null, onHover: (code: string | null) => void, isUnlocked: boolean }) {
  ...
  useEffect(() => {
    ...
    ctx.beginPath(); for (let i = 0; i < total; i++) { const angle = i * angleStep - Math.PI / 2, r = (radius * results[i].score) / 5, x = cx + r * Math.cos(angle), y = cy + r * Math.sin(angle); if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y) }; ctx.closePath(); ctx.fillStyle = 'rgba(29,158,117,0.2)'; ctx.fill(); ctx.strokeStyle = '#1d9e75'; ctx.lineWidth = 2.5; ctx.stroke()
    for (let i = 0; i < total; i++) { const angle = i * angleStep - Math.PI / 2, r = (radius * results[i].score) / 5, x = cx + r * Math.cos(angle), y = cy + r * Math.sin(angle), isHovered = hoveredCode === results[i].code; ctx.beginPath(); ctx.arc(x, y, isHovered ? 8 : 5, 0, Math.PI * 2); ctx.fillStyle = isHovered ? '#1a5276' : '#1d9e75'; ctx.fill(); if (isHovered) { ctx.strokeStyle = 'white'; ctx.lineWidth = 2; ctx.stroke() } }
    ctx.font = '11px sans-serif'; for (let i = 0; i < total; i++) { const angle = i * angleStep - Math.PI / 2, r = radius + 28, x = cx + r * Math.cos(angle), y = cy + r * Math.sin(angle); ctx.textAlign = x < cx - 10 ? 'right' : x > cx + 10 ? 'left' : 'center'; ctx.fillStyle = hoveredCode === results[i].code ? '#1a5276' : '#333'; ctx.font = hoveredCode === results[i].code ? 'bold 11px sans-serif' : '11px sans-serif'; ctx.fillText(results[i].code, x, y + 4) }
  }, [results, hoveredCode])
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'white', borderRadius: '12px', width: '100%', maxWidth: '520px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ background: '#0F2744', color: 'white', padding: '20px 24px', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div><div style={{ fontSize: '12px', color: '#4fa3e0', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>👤 FPI Consulting</div><div style={{ fontSize: '16px', fontWeight: '700' }}>Request a Consultation</div></div>
...
export default function ResultsR2RPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      const unlocked = profileData?.is_admin === true || (profileData?.unlocked_processes || []).includes('Record to Report')
      ...
      const effortResult = await supabase.from('process_effort').select('*').eq('user_id', user.id).eq('process_name', 'Record to Report')
      ...
      if (unlocked) {
        ...
        if (scoredResults.length > 0) {
          ...
          try {
            const response = await fetch('/api/generate-insights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ l2Results: scoredResults, processName: 'Record to Report', effortData: effortData.filter((r: any) => r.step_code !== 'roi_settings'), hourlyRate: roiRow?.hourly_rate || 0, savingPercent: roiRow?.saving_percent || 0 }) })
  ...
  const saveROISettings = async (rate: number, percent: number) => {
    ...
    await supabase.from('process_effort').upsert({ user_id: user.id, process_name: 'Record to Report', step_code: 'roi_settings', step_name: 'ROI Settings', hourly_rate: rate, saving_percent: percent, updated_at: new Date().toISOString() }, { onConflict: 'user_id,process_name,step_code' })
  }

  if (loading) return (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'sans-serif' }}><div style={{ textAlign: 'center' }}><div style={{ fontSize: '24px', marginBottom: '12px' }}>⏳</div><div style={{ fontSize: '16px', color: '#666' }}>Loading your R2R results...</div></div></div>)
  ...
  const keyFindings = aiInsightsData?.keyFindings || [{ type: 'strength', text: 'Your highest scoring R2R processes show structured approaches.' }, { type: 'gap', text: 'Lower scoring processes need formalisation.' }, { type: 'opportunity', text: 'Significant opportunity through automation and process governance.' }]
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'sans-serif', background: '#f4f6f9' }}>
      <div className="res-header" style={{ background: '#0F2744', color: 'white', padding: '32px 40px' }}>
        ...
        {!isUnlocked && (
          <div style={{ marginTop: '16px', background: 'rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '20px' }}>🔒</span>
            <div><div style={{ fontSize: '14px', fontWeight: '700', color: 'white', marginBottom: '2px' }}>Full results available</div><div style={{ fontSize: '12px', color: '#a0c4e8' }}>Get in touch to unlock your full results and accelerate your transformation</div></div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '32px 40px', background: '#f4f6f9' }}>
        {activeTab === 'overview' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div><h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e' }}>{viewMode === 'grid' ? 'Capability Maturity Grid — Record to Report' : 'Maturity Spider Chart — Record to Report'}</h2><p style={{ fontSize: '13px', color: '#666', marginTop: '4px' }}>Hover over each dot to see the detailed maturity narrative</p></div>
            ...
            {viewMode === 'grid' && (
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', marginBottom: '24px', position: 'relative' }}>
                <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                  {[{ label: 'Initial (1–1.9)', color: '#ef4444' }, { label: 'Repeatable (2–2.9)', color: '#f97316' }, { label: 'Defined (3–3.9)', color: '#eab308' }, { label: 'Managed (4–4.9)', color: '#22c55e' }, { label: 'Optimised (5.0)', color: '#3b82f6' }].map(l => (<div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '10px', height: '10px', borderRadius: '50%', background: l.color }} /><span style={{ fontSize: '12px', color: '#666' }}>{l.label}</span></div>))}
                </div>
                {hoveredResult && isUnlocked && (<div style={{ position: 'fixed', left: tooltipPos.x + 16, top: tooltipPos.y - 10, background: '#0F2744', color: 'white', padding: '14px 16px', borderRadius: '8px', maxWidth: '320px', zIndex: 1000, boxShadow: '0 8px 24px rgba(0,0,0,0.2)', pointerEvents: 'none' }}><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}><span style={{ background: '#1d9e75', padding: '2px 6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>{hoveredResult.code}</span><span style={{ fontWeight: '700', fontSize: '13px' }}>{hoveredResult.name}</span></div><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}><span style={{ padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700', background: getLevelColor(hoveredResult.level), color: 'white' }}>{hoveredResult.level}</span><span style={{ fontSize: '13px', color: '#a0c4e8' }}>Score: {hoveredResult.score} / 5.0</span></div><p style={{ fontSize: '12px', color: '#c8dff0', lineHeight: '1.6', margin: 0 }}>{generatingInsights ? 'Generating AI narrative...' : hoveredResult.narrative}</p></div>)}
                {hoveredResult && !isUnlocked && (<div style={{ position: 'fixed', left: tooltipPos.x + 16, top: tooltipPos.y - 10, background: '#0F2744', color: 'white', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', zIndex: 100, pointerEvents: 'none', maxWidth: '220px' }}><div style={{ fontWeight: '700', marginBottom: '4px' }}>{hoveredResult.code} — {hoveredResult.name}</div><div style={{ color: getLevelColor(hoveredResult.level), fontWeight: '600', marginBottom: '6px' }}>{hoveredResult.score} / 5.0 — {hoveredResult.level}</div><div style={{ fontSize: '12px', color: '#f97316' }}>🔒 Unlock to view AI narrative</div></div>)}
                <div style={{ display: 'grid', gridTemplateColumns: '220px repeat(5, 1fr)', marginBottom: '4px' }}>
                  ...
                  {maturityColumns.map(col => (<div key={col.key} style={{ padding: '10px 12px', fontSize: '11px', fontWeight: '700', color: col.color, textAlign: 'center', background: col.bg, borderRadius: '4px', margin: '0 2px' }}><div>{col.label}</div><div style={{ fontWeight: '400', color: '#999', fontSize: '10px', marginTop: '2px' }}>{col.range}</div></div>))}
                </div>
                {l2Results.map((r, idx) => (
                  <div key={r.code} style={{ display: 'grid', gridTemplateColumns: '220px repeat(5, 1fr)', background: idx % 2 === 0 ? '#fafafa' : 'white', borderRadius: '4px', marginBottom: '2px' }}>
                    <div style={{ padding: '14px 12px' }} onMouseEnter={e => { setHoveredCode(r.code); setTooltipPos({ x: e.clientX, y: e.clientY }) }} onMouseLeave={() => setHoveredCode(null)}>
                    ...
                    {maturityColumns.map((col, colIdx) => (
                      <div key={col.key} style={{ padding: '14px 12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onMouseEnter={e => { setHoveredCode(r.code); setTooltipPos({ x: e.clientX, y: e.clientY }) }} onMouseLeave={() => setHoveredCode(null)}>
                        {r.score > 0 && getScorePosition(r.score) === colIdx && (
                          <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: getLevelColor(r.level), display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 2px 8px ${getLevelColor(r.level)}60` }}>
            ...
            {viewMode === 'spider' && (
              <div style={{ background: 'white', borderRadius: '12px', padding: '32px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', marginBottom: '24px', position: 'relative' }}>
                ...
                {hoveredResult && isUnlocked && (<div style={{ position: 'absolute', top: '50%', right: '24px', transform: 'translateY(-50%)', background: '#0F2744', color: 'white', padding: '16px', borderRadius: '8px', width: '260px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}><span style={{ background: '#1d9e75', padding: '2px 6px', borderRadius: '4px', fontSize: '11px', fontWeight: 'bold' }}>{hoveredResult.code}</span><span style={{ fontWeight: '700', fontSize: '13px' }}>{hoveredResult.name}</span></div><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}><span style={{ padding: '2px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700', background: getLevelColor(hoveredResult.level), color: 'white' }}>{hoveredResult.level}</span><span style={{ fontSize: '13px', color: '#a0c4e8' }}>{hoveredResult.score} / 5.0</span></div><p style={{ fontSize: '12px', color: '#c8dff0', lineHeight: '1.6', margin: 0 }}>{generatingInsights ? 'Generating AI narrative...' : hoveredResult.narrative}</p></div>)}
                {hoveredResult && !isUnlocked && (<div style={{ position: 'fixed', left: tooltipPos.x + 12, top: tooltipPos.y - 40, background: '#0F2744', color: 'white', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', zIndex: 100, pointerEvents: 'none', maxWidth: '220px' }}><div style={{ fontWeight: '700', marginBottom: '4px' }}>{hoveredResult.code} — {hoveredResult.name}</div><div style={{ color: getLevelColor(hoveredResult.level), fontWeight: '600', marginBottom: '6px' }}>{hoveredResult.score} / 5.0 — {hoveredResult.level}</div><div style={{ fontSize: '12px', color: '#f97316' }}>🔒 Unlock to view AI narrative</div></div>)}
              </div>
            )}
            ...
            <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', marginBottom: '24px' }}>
              ...
              {keyFindings.map((f, i) => (<div key={i} style={{ display: 'flex', gap: '12px', marginBottom: '10px', padding: '12px', borderRadius: '8px', background: f.type === 'strength' ? '#f0fdf4' : f.type === 'gap' ? '#fef2f2' : '#fffbeb' }}><span style={{ fontSize: '16px', flexShrink: 0 }}>{f.type === 'strength' ? '✅' : f.type === 'gap' ? '⚠️' : '💡'}</span><span style={{ fontSize: '13px', color: '#333', lineHeight: '1.6' }}>{f.text}</span></div>))}
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                {showBenchmarkInfo && (<div style={{ marginTop: '12px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <p style={{ fontSize: '13px', color: '#444', lineHeight: '1.7', marginBottom: '12px' }}>These indicative benchmarks are drawn from Arpero's Finance transformation experience across multiple sectors, combined with AI-modelled industry data. They are designed to provide directional guidance rather than precise comparisons.</p>
                  <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}><div style={{ padding: '8px 14px', background: '#e8f4fd', borderRadius: '6px', fontSize: '12px', color: '#0F4C81' }}>📊 Drawn from Arpero's consulting experience</div><div style={{ padding: '8px 14px', background: '#e8f4fd', borderRadius: '6px', fontSize: '12px', color: '#0F4C81' }}>🤖 Enhanced with AI-modelled industry patterns</div><div style={{ padding: '8px 14px', background: '#e8f4fd', borderRadius: '6px', fontSize: '12px', color: '#0F4C81' }}>📊 Indicative — for directional guidance only</div></div>
        ...
        {activeTab === 'l2breakdown' && (isUnlocked ? (
          <div><h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e', marginBottom: '20px' }}>L3 Breakdown by L2 Process</h3><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>{l2Results.map(r => (<div key={r.code} style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}><div><div style={{ fontSize: '11px', color: '#1d9e75', fontWeight: '700' }}>L2 {r.code}</div><div style={{ fontSize: '15px', fontWeight: '700', color: '#1a1a2e' }}>{r.name}</div></div><div style={{ textAlign: 'right' }}><span style={{ padding: '3px 12px', borderRadius: '12px', fontSize: '12px', fontWeight: '700', background: getLevelColor(r.level), color: 'white' }}>{r.level}</span><div style={{ fontSize: '18px', fontWeight: 'bold', color: getLevelColor(r.level), marginTop: '4px' }}>{r.score}</div></div></div><div style={{ background: '#f0f0f0', borderRadius: '4px', height: '8px', marginBottom: '16px' }}><div style={{ width: `${(r.score / 5) * 100}%`, background: getLevelColor(r.level), height: '100%', borderRadius: '4px' }} /></div>{r.l3s.filter(l => l.score > 0).map((l3, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}><span style={{ fontSize: '11px', color: '#1d9e75', fontWeight: '600', width: '40px', flexShrink: 0 }}>{l3.code}</span><div style={{ flex: 1, background: '#f0f0f0', borderRadius: '4px', height: '6px' }}><div style={{ width: `${(l3.score / 5) * 100}%`, background: getLevelColor(l3.level), height: '100%', borderRadius: '4px' }} /></div><span style={{ fontSize: '12px', fontWeight: '700', color: getLevelColor(l3.level), width: '25px', textAlign: 'right' }}>{l3.score}</span></div>))}{r.l3s.filter(l => l.score > 0).length === 0 && (<div style={{ fontSize: '13px', color: '#999', fontStyle: 'italic' }}>No responses recorded yet</div>)}</div>))}</div></div>
        ) : <GatedOverlay processName="Record to Report" />)}

        {activeTab === 'aiinsights' && (isUnlocked ? (
          <div>
            <div style={{ background: '#0F2744', borderRadius: '12px', padding: '24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}><div style={{ width: '48px', height: '48px', background: '#1d9e75', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', flexShrink: 0 }}>🤖</div><div><div style={{ fontSize: '18px', fontWeight: '700', color: 'white', marginBottom: '4px' }}>AI-Powered Maturity Insights — Record to Report</div><div style={{ fontSize: '13px', color: '#7db3e8' }}>{generatingInsights ? '⏳ Generating AI insights...' : 'Generated by FPI Intelligence · Assessment completed today'}</div></div></div>
      ...
      {showBenchmarkChat && isUnlocked && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '12px', width: '560px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div><h3 style={{ fontSize: '16px', fontWeight: '700', color: '#1a1a2e', margin: 0 }}>🤖 Ask AI about your benchmark</h3><p style={{ fontSize: '12px', color: '#666', margin: '4px 0 0' }}>Powered by FPI Finance Intelligence</p></div><button onClick={() => setShowBenchmarkChat(false)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#666' }}>✕</button></div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: '400px' }}>
              {benchmarkChatMessages.length === 0 && (<div style={{ fontSize: '13px', color: '#666', fontStyle: 'italic' }}>Ask me anything about your R2R benchmark results.</div>)}
