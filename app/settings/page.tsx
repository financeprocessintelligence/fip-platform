...
export default function SettingsPage() {
  ...
  const saveProfile = async () => {
    setSaving(true)
    const { error } = await supabase.auth.updateUser({ data: { full_name: fullName, job_title: jobTitle } })
    setSaving(false)
  ...
  const saveOrganisation = async () => {
    setSaving(true)
    const { error } = await supabase.auth.updateUser({ data: { org_name: orgName, industry, org_size: orgSize } })
    setSaving(false)
  ...
  const savePassword = async () => {
    ...
    else { showMessage('success', 'Password updated successfully.'); setNewPassword(''); setConfirmPassword('') }
  }
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '24px', marginBottom: '12px' }}>⏳</div><div style={{ fontSize: '16px', color: '#666' }}>Loading settings...</div></div>
    </div>
  )
    ...
    <>
      <style>{`
        @media (max-width: 768px) {
          .set-sidebar { transform: translateX(-100%); position: fixed !important; z-index: 200; height: 100vh; transition: transform 0.3s; overflow-y: auto; }
          .set-sidebar.open { transform: translateX(0); }
          .set-topbar { display: flex !important; }
          .set-main { padding: 16px !important; }
          .set-layout { grid-template-columns: 1fr !important; }
          .set-secnav { display: flex !important; flex-direction: row !important; overflow-x: auto !important; gap: 8px !important; padding: 8px !important; }
          .set-secnav-item { white-space: nowrap !important; }
        }
        @media (min-width: 769px) {
          .set-topbar { display: none !important; }
        }
      `}</style>
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'sans-serif' }}>
        ...
        {/* Sidebar */}
        <div className={`set-sidebar${sidebarOpen ? ' open' : ''}`} style={{ width: '240px', background: '#0F4C81', color: 'white', padding: '24px 16px', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Main */}
        <div className="set-main" style={{ flex: 1, background: '#f4f6f9', padding: '32px', overflowY: 'auto' }}>
          {/* Mobile topbar */}
          <div className="set-topbar" style={{ display: 'none', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', background: '#0F4C81', padding: '12px 16px', borderRadius: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'white', fontWeight: 'bold', fontSize: '14px' }}>Settings</span>
            </div>
          ...
          <div className="set-layout" style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '24px' }}>
            {/* Section Nav */}
            <div className="set-secnav" style={{ background: 'white', borderRadius: '12px', padding: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', height: 'fit-content' }}>
              {sections.map(s => (
                <div key={s.key} className="set-secnav-item" onClick={() => setActiveSection(s.key)} style={{ padding: '10px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: activeSection === s.key ? '600' : '400', color: activeSection === s.key ? '#0F4C81' : '#555', background: activeSection === s.key ? '#e8f4fd' : 'transparent', marginBottom: '4px' }}>
                  {s.label}
                </div>
              ))}
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              ...
              {activeSection === 'profile' && (
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px', padding: '16px', background: '#f4f6f9', borderRadius: '8px' }}>
                    <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: '#0F4C81', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', color: 'white', fontWeight: 'bold', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div><label style={labelStyle}>Full Name</label><input value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Your full name" style={inputStyle} /></div>
                    <div><label style={labelStyle}>Job Title</label><input value={jobTitle} onChange={e => setJobTitle(e.target.value)} placeholder="e.g. Head of FP&A" style={inputStyle} /></div>
                    <div>
              ...
              {activeSection === 'organisation' && (
                <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div><label style={labelStyle}>Organisation Name</label><input value={orgName} onChange={e => setOrgName(e.target.value)} placeholder="Your organisation name" style={inputStyle} /></div>
                    <div>
              ...
              {activeSection === 'password' && (
                <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div><label style={labelStyle}>New Password</label><input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Minimum 8 characters" style={inputStyle} /></div>
