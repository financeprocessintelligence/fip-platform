            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div>
                  ...
                  <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="e.g. We need help prioritising our improvement roadmap and building the business case..." style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', minHeight: '80px', resize: 'vertical', boxSizing: 'border-box', color: '#333' }} />
                </div>
...
function ResultsPageInner() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      const { data, error } = await supabase.from('assessments').select('*').eq('user_id', targetUserId).eq('process_name', 'Plan to Perform')
      ...
      const effortResult = await supabase.from('process_effort').select('*').eq('user_id', targetUserId).eq('process_name', 'Plan to Perform')
...
export default function ResultsPage() {
  return (
    <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}><div>Loading...</div></div>}>
      <ResultsPageInner />
    </Suspense>
  )
}
