...
const maturityColumns = [
  { key: 'Initial', range: '1–1.9', desc: 'Manual, reactive, no documentation', color: '#ef4444' },
  { key: 'Repeatable', range: '2–2.9', desc: 'Some standardisation, roles emerging', color: '#f97316' },
  { key: 'Defined', range: '3–3.9', desc: 'Documented, KPIs in place', color: '#eab308' },
  { key: 'Managed', range: '4–4.9', desc: 'Data-driven, cross-functional', color: '#22c55e' },
  { key: 'Optimised', range: '5.0', desc: 'Predictive, AI-assisted', color: '#3b82f6' },
]
...
export default function ResultsPrintPage() {
      ...
      if (scored.length > 0) {
        ...
        try {
          const res = await fetch('/api/generate-insights', {
            ...
            body: JSON.stringify({ l2Results: scored, processName: 'Plan to Perform' })
          })
          const aiData = await res.json()
          if (aiData.success) {
  console.log('AI insights keys:', Object.keys(aiData.insights))
 ...
 const handleDownloadPDF = async () => {
    ...
    pdf.save('plan-to-perform-report.pdf')
  }
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', background: '#f4f6f9', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#0F2744', color: 'white', padding: '16px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ fontSize: '16px', fontWeight: '700' }}>Plan to Perform — Full Report</div>
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => router.push('/results')} style={{ padding: '9px 16px', background: 'rgba(255,255,255,0.1)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '6px', fontSize: '13px', cursor: 'pointer' }}>← Back to Results</button>
        </div>
      </div>

      <div id="print-content" style={{ padding: '40px', maxWidth: '900px', margin: '0 auto', pageBreakInside: 'avoid' }}>
        {/* Header */}
        <div style={{ background: '#0F2744', color: 'white', borderRadius: '12px', padding: '32px', marginBottom: '60px' }}>
          ...
          <h1 style={{ fontSize: '26px', fontWeight: 'bold', marginBottom: '6px' }}>Plan to Perform — Maturity Report</h1>
        ...
        {/* Overall Score */}
        <div style={{ background: 'white', borderRadius: '12px', padding: '24px', marginBottom: '40px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', textAlign: 'center', pageBreakAfter: 'always' }}>
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '24px', marginBottom: '60px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
            <thead>
              <tr>
                <th style={{ padding: '8px', textAlign: 'left', background: '#f4f6f9', borderRadius: '4px' }}>Process</th>
