        ...
        <div style={{ padding: '24px' }}>
          {submitted ? (
            <div style={{ textAlign: 'center', padding: '24px' }}>
              ...
              <p style={{ fontSize: '14px', color: '#666' }}>One of our FPI consultants will be in touch within 24 hours to discuss your {processName} assessment results.</p>
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. John Smith" style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', boxSizing: 'border-box', color: '#333' }} />
                </div>
                <div>
                  <label style={{ fontSize: '13px', fontWeight: '600', color: '#333', display: 'block', marginBottom: '6px' }}>Email Address *</label>
                  <input value={email} onChange={e => setEmail(e.target.value)} placeholder="e.g. john@company.com" style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', boxSizing: 'border-box', color: '#333' }} />
                </div>
                <div>
                  <label style={{ fontSize: '13px', fontWeight: '600', color: '#333', display: 'block', marginBottom: '6px' }}>Availability</label>
                  <input value={availability} onChange={e => setAvailability(e.target.value)} placeholder="e.g. Weekday mornings, any time next week" style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', boxSizing: 'border-box', color: '#333' }} />
                </div>
                <div>
                  <label style={{ fontSize: '13px', fontWeight: '600', color: '#333', display: 'block', marginBottom: '6px' }}>What would you like to discuss?</label>
                  <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="e.g. We need help prioritising our PtP improvement roadmap..." style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', minHeight: '80px', resize: 'vertical', boxSizing: 'border-box', color: '#333' }} />
                </div>
...
export default function ResultsPtPPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      if (scoredResults.length > 0) {
        ...
        try {
          const response = await fetch('/api/generate-insights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ l2Results: scoredResults, processName: 'Procure to Pay', effortData: filteredEffort, hourlyRate: loadedHourlyRate, savingPercent: loadedSavingPercent }) })
  ...
  const keyFindings = aiInsightsData?.keyFindings || [{ type: 'strength', text: 'Your highest scoring processes show structured approaches.' }, { type: 'gap', text: 'Lower scoring processes need formalisation.' }, { type: 'opportunity', text: 'Opportunity to improve through better tooling and automation.' }]
  ...
  const handleBenchmarkChat = async () => {
    ...
    const response = await fetch('/api/benchmark-chat', {
      ...
      body: JSON.stringify({ messages: updatedMessages, benchmarks, overallScore })
    })
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'sans-serif', background: '#f4f6f9' }}>
      <div style={{ background: '#0F2744', color: 'white', padding: '32px 40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h1 style={{ fontSize: '28px', fontWeight: 'bold', marginBottom: '6px' }}>Procure to Pay — Maturity Report</h1>
            <p style={{ color: '#a0c4e8', fontSize: '14px' }}>Finance Process Intelligence Platform · Assessment completed today · Confidential</p>
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={() => router.push('/results-ptp-print')} style={{ padding: '9px 16px', background: '#1d9e75', color: 'white', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>⬇ Download PDF Report</button>
        ...
        {activeTab === 'overview' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e' }}>{viewMode === 'grid' ? 'Capability Maturity Grid — Procure to Pay' : 'Maturity Spider Chart — Procure to Pay'}</h2>
                ...
                {l2Results.map((result, i) => (
                  <div key={result.code} style={{ display: 'grid', gridTemplateColumns: '200px repeat(5, 1fr)', borderBottom: i < l2Results.length - 1 ? '1px solid #f0f0f0' : 'none' }}
                    ...
                    {maturityColumns.map((col, j) => {
                      ...
                      return (
                        <div key={col.key} style={{ padding: '14px 8px', background: isActive ? col.bg : hoveredCode === result.code ? '#f8f9fb' : 'white', borderLeft: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {isActive && (
                ...
                {showBenchmarkInfo && (
                  <div style={{ marginTop: '12px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <p style={{ fontSize: '13px', color: '#444', lineHeight: '1.7', marginBottom: '12px' }}>
                      Benchmarks are derived from FPI's proprietary Finance Maturity Index, combining primary research across 200+ Finance functions and secondary analysis of industry performance data.
                    </p>
        ...
        {activeTab === 'aiinsights' && (
          <div>
            <div style={{ background: '#0F2744', borderRadius: '12px', padding: '24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '18px', fontWeight: '700', color: 'white', marginBottom: '4px' }}>AI-Powered Maturity Insights — Procure to Pay</div>
          ...
          <div>
            <div style={{ background: 'linear-gradient(135deg, #0F2744 0%, #0F4C81 100%)', borderRadius: '12px', padding: '24px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#a0c4e8' }}>Our FPI consultants can help you implement your PtP improvement roadmap.</div>
              </div>
          ...
          <div style={{ background: 'white', borderRadius: '12px', width: '560px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ fontSize: '12px', color: '#666', margin: '4px 0 0' }}>Powered by FPI Intelligence</p>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: '300px' }}>
              {benchmarkChatMessages.length === 0 && (
                <div style={{ fontSize: '13px', color: '#666', fontStyle: 'italic' }}>Ask me anything about your benchmark — e.g. "Why am I below average in Invoice Processing?" or "What should I focus on to improve my touchless rate?"</div>
              )}
              {benchmarkChatMessages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '80%', padding: '10px 14px', borderRadius: '10px', fontSize: '13px', lineHeight: '1.6', background: m.role === 'user' ? '#0F4C81' : '#f4f6f9', color: m.role === 'user' ? 'white' : '#1a1a2e' }}>{m.content}</div>
                </div>
              ))}
