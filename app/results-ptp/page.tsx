...
const stepDefinitions = [
  { code: '1.1', name: 'Supplier Management' },
  { code: '1.2', name: 'Requisitioning' },
  { code: '1.3', name: 'Purchasing' },
  { code: '1.4', name: 'Receiving' },
  { code: '1.5', name: 'Invoice Processing & Payment' },
  { code: '1.6', name: 'Cash & Payment Management' },
  { code: '1.7', name: 'P-Card & T&E Administration' },
  { code: '1.8', name: 'Purchasing/Payment Inquiries' },
  { code: '1.9', name: 'Compliance & Controls' },
  { code: '1.10', name: 'Period End Close' },
  { code: '1.11', name: 'Reporting & Analytics' },
  { code: '1.12', name: 'Manage Process' },
  { code: '1.13', name: 'System Governance' },
]

const benchmarkAverages: Record<string, number> = {
  '1.1': 2.3, '1.2': 2.5, '1.3': 2.6, '1.4': 2.4, '1.5': 2.7, '1.6': 2.2,
  '1.7': 2.4, '1.8': 2.3, '1.9': 2.5, '1.10': 2.6, '1.11': 2.3, '1.12': 2.1, '1.13': 2.2,
  'overall': 2.4
}
...
const defaultRecommendations = [
  { priority: '1', action: 'Implement Automated Three-Way Matching', detail: 'Deploy automated PO, GR and invoice matching to reduce manual AP effort and improve touchless processing rates.', impact: 'High', effort: 'Low', timeline: '1 Quarter', owner: 'AP Manager / IT', l2: '1.5' },
  { priority: '2', action: 'Establish a Formal Supplier Onboarding Process', detail: 'Introduce a structured digital onboarding workflow with automated risk and compliance checks.', impact: 'High', effort: 'Medium', timeline: '2 Quarters', owner: 'Procurement Director', l2: '1.1' },
  { priority: '3', action: 'Deploy a Self-Service Supplier Portal', detail: 'Reduce supplier payment query volumes by giving suppliers real-time visibility of invoice and payment status.', impact: 'High', effort: 'Medium', timeline: '2 Quarters', owner: 'AP Manager', l2: '1.8' },
  { priority: '4', action: 'Implement Continuous Controls Monitoring', detail: 'Move from periodic audit-based controls testing to continuous automated monitoring of key PtP controls.', impact: 'Medium', effort: 'Medium', timeline: '3 Quarters', owner: 'Financial Controller', l2: '1.9' },
  { priority: '5', action: 'Optimise Payment Terms Strategy', detail: 'Conduct a working capital analysis across the supplier base and implement dynamic discounting where beneficial.', impact: 'Medium', effort: 'Low', timeline: '1 Quarter', owner: 'Treasury / CFO', l2: '1.6' },
]
...
export default function ResultsPtPPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      const unlocked = profileData?.is_admin === true || (profileData?.unlocked_processes || []).includes('Procure to Pay')
      setIsUnlocked(unlocked)
      const { data, error } = await supabase.from('assessments').select('*').eq('user_id', user.id).eq('process_name', 'Procure to Pay')
      ...
      const effortResult = await supabase.from('process_effort').select('*').eq('user_id', user.id).eq('process_name', 'Procure to Pay')
      ...
      if (unlocked) {
        ...
        if (scoredResults.length > 0) {
          ...
          try {
            const response = await fetch('/api/generate-insights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ l2Results: scoredResults, processName: 'Procure to Pay', effortData: effortData.filter((r: any) => r.step_code !== 'roi_settings'), hourlyRate: roiRow?.hourly_rate || 0, savingPercent: roiRow?.saving_percent || 0 }) })
  ...
  const saveROISettings = async (rate: number, percent: number) => {
    ...
    await supabase.from('process_effort').upsert({ user_id: user.id, process_name: 'Procure to Pay', step_code: 'roi_settings', step_name: 'ROI Settings', hourly_rate: rate, saving_percent: percent, updated_at: new Date().toISOString() }, { onConflict: 'user_id,process_name,step_code' })
  }
  ...
  const benchmarks = [{ label: 'Your overall score', score: overallScore, avg: benchmarkAverages['overall'] }, ...l2Results.filter(r => r.score > 0).map(r => ({ label: `${r.code} ${r.name}`, score: r.score, avg: benchmarkAverages[r.code] || 2.4 }))]
  const keyFindings = aiInsightsData?.keyFindings || [{ type: 'strength', text: 'Your highest scoring processes show structured approaches to procurement.' }, { type: 'gap', text: 'Lower scoring processes need formalisation and better tooling.' }, { type: 'opportunity', text: 'Significant opportunity through touchless processing and automation.' }]
