...
const defaultRecommendations = [
  { priority: '1', action: 'Implement Earned Value Management (EVM)', detail: 'Introduce a structured EVM framework to track planned vs actual vs earned value across the project portfolio, enabling earlier identification of cost and schedule variances.', impact: 'High', effort: 'Medium', timeline: '2 Quarters', owner: 'PMO / Project Accounting Manager', l2: '1.3' },
  { priority: '2', action: 'Integrate Project Spend into Rolling Forecast', detail: 'Connect project actuals and commitments to the FP&A rolling forecast in real time, eliminating manual data transfer and improving forecast accuracy.', impact: 'High', effort: 'Medium', timeline: '2 Quarters', owner: 'Head of FP&A / Project Finance', l2: '1.7' },
  { priority: '3', action: 'Establish Formal Benefits Realisation Tracking', detail: 'Implement a benefits realisation framework to track planned vs actual project benefits post-completion, ensuring investment decisions are evidence-based.', impact: 'High', effort: 'Low', timeline: '1 Quarter', owner: 'CFO / Project Sponsor', l2: '1.7' },
  { priority: '4', action: 'Automate AUC to Fixed Asset Transfer', detail: 'Deploy rules-based automation to trigger asset under construction (AUC) to fixed asset transfers on project completion, reducing manual effort and capitalisation errors.', impact: 'Medium', effort: 'Low', timeline: '1 Quarter', owner: 'Financial Controller', l2: '1.6' },
  { priority: '5', action: 'Implement AI-Powered Project Cost Forecasting', detail: 'Use ML models to predict estimate at completion (EAC) based on project burn rates and EVM data, improving forecast accuracy and reducing Finance effort.', impact: 'Medium', effort: 'Medium', timeline: '3 Quarters', owner: 'Head of Project Finance / IT', l2: '1.9' },
]
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div>
                  ...
                  <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="e.g. We need help prioritising our project accounting improvement roadmap..." style={{ width: '100%', padding: '10px 12px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', minHeight: '80px', resize: 'vertical', boxSizing: 'border-box', color: '#333' }} />
                </div>
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      if (scoredResults.length > 0) {
        ...
        try {
          const response = await fetch('/api/generate-insights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ l2Results: scoredResults, processName: 'Project to Result', effortData: filteredEffort, hourlyRate: loadedHourlyRate, savingPercent: loadedSavingPercent }) })
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'sans-serif', background: '#f4f6f9' }}>
      <div style={{ background: '#0F2744', color: 'white', padding: '32px 40px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h1 style={{ fontSize: '28px', fontWeight: 'bold', marginBottom: '6px' }}>Project to Result — Maturity Report</h1>
            <p style={{ color: '#a0c4e8', fontSize: '14px' }}>Finance Process Intelligence Platform · Assessment completed today · Confidential</p>
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={() => router.push('/results-p2r-print')} style={{ padding: '9px 16px', background: '#1d9e75', color: 'white', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>⬇ Download PDF Report</button>
        ...
        {activeTab === 'overview' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e' }}>{viewMode === 'grid' ? 'Capability Maturity Grid — Project to Result' : 'Maturity Spider Chart — Project to Result'}</h2>
            ...
            <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              ...
              <p style={{ fontSize: '13px', color: '#666', marginBottom: '12px' }}>How your Project to Result maturity compares to your industry peers at a similar organisational scale</p>
              <div style={{ marginBottom: '20px' }}>
                ...
                {showBenchmarkInfo && (
                  <div style={{ marginTop: '12px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    <p style={{ fontSize: '13px', color: '#444', lineHeight: '1.7', marginBottom: '12px' }}>Benchmarks are derived from FPI's proprietary Finance Maturity Index, combining primary research across 200+ Finance functions and secondary analysis of industry performance data.</p>
        ...
        {activeTab === 'aiinsights' && (
          <div>
            <div style={{ background: '#0F2744', borderRadius: '12px', padding: '24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '18px', fontWeight: '700', color: 'white', marginBottom: '4px' }}>AI-Powered Maturity Insights — Project to Result</div>
          ...
          <div>
            <div style={{ background: 'linear-gradient(135deg, #0F2744 0%, #0F4C81 100%)', borderRadius: '12px', padding: '24px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#a0c4e8' }}>Our FPI consultants can help you implement your Project to Result improvement roadmap.</div>
              </div>
      ...
      {activeChatRec && (<RecommendationChat recommendation={activeChatRec} processName="Project to Result" score={overallScore} onClose={() => setActiveChatRec(null)} />)}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '12px', width: '560px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: '300px' }}>
              {benchmarkChatMessages.length === 0 && (<div style={{ fontSize: '13px', color: '#666', fontStyle: 'italic' }}>Ask me anything about your benchmark — e.g. "Why am I below average in project execution?" or "What should I focus on to improve my EVM maturity?"</div>)}
