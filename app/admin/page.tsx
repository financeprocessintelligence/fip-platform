...
export default function AdminPage() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <div style={{ textAlign: 'center' }}><div style={{ fontSize: '24px', marginBottom: '12px' }}>⏳</div><div style={{ fontSize: '16px', color: '#666' }}>Loading admin dashboard...</div></div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'sans-serif', background: '#f4f6f9' }}>
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #e0e4ea', padding: '0 40px', display: 'flex' }}>
        {['overview', 'users', 'leads', 'assessments'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '14px 20px', border: 'none', background: 'transparent', fontSize: '14px', fontWeight: activeTab === tab ? '700' : '400', color: activeTab === tab ? '#0F4C81' : '#666', borderBottom: activeTab === tab ? '2px solid #0F4C81' : '2px solid transparent', cursor: 'pointer' }}>
            {tab === 'overview' ? 'Overview' : tab === 'users' ? `Users (${users.length})` : tab === 'leads' ? `Leads (${leads.length})` : 'Assessments'}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '32px 40px' }}>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '16px', marginBottom: '32px' }}>
              {[
                ...
                { label: 'Total Leads', value: stats.totalLeads.toString(), color: '#8b5cf6' },
                { label: 'Unlock Requests', value: stats.unlockRequests.toString(), color: '#ef4444' },
              ].map(s => (
        ...
        {activeTab === 'users' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <input type="text" placeholder="Search by name, email or organisation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ padding: '9px 14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', width: '300px', color: '#333' }} />
            </div>
        ...
        {activeTab === 'leads' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a1a2e' }}>All Leads ({leads.length})</h2>
              <div style={{ display: 'flex', gap: '8px' }}>
                {(['all', 'registration', 'unlock_request'] as const).map(f => (
                  <button key={f} onClick={() => setLeadsFilter(f)} style={{ padding: '8px 16px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '13px', fontWeight: leadsFilter === f ? '700' : '400', background: leadsFilter === f ? '#0F4C81' : 'white', color: leadsFilter === f ? 'white' : '#666', cursor: 'pointer' }}>
                    {f === 'all' ? 'All' : f === 'registration' ? 'Registrations' : 'Unlock Requests'}
                  </button>
                ))}
              </div>
            </div>
