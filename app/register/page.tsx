...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    fullName: '', jobTitle: '', email: '', password: '', roleType: '',
    orgName: '', industry: '', orgSize: '', currency: '£', revenue: '',
  })
  ...
  const handleSubmit = async () => {
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          full_name: form.fullName, job_title: form.jobTitle, role_type: form.roleType,
          org_name: form.orgName, industry: form.industry, org_size: form.orgSize,
    ...
    if (error) {
      ...
      await supabase.from('leads').insert({
        full_name: form.fullName, email: form.email, job_title: form.jobTitle,
        org_name: form.orgName, industry: form.industry, org_size: form.orgSize,
        revenue: form.revenue ? `${form.currency} ${form.revenue}` : '',
        role_type: form.roleType, source: 'registration'
      })
      router.push('/?registered=true')
    }
  }

  const inputStyle: React.CSSProperties = { display: 'block', width: '100%', marginBottom: '12px', padding: '10px 14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', color: '#1a1a2e', background: 'white', boxSizing: 'border-box' }
  const labelStyle: React.CSSProperties = { display: 'block', fontSize: '13px', fontWeight: '600', color: '#444', marginBottom: '4px' }
    ...
    <>
      <style>{`
        @media (max-width: 768px) {
          ...
          .reg-sidebar { width: 100% !important; padding: 20px 24px !important; flex-direction: row !important; align-items: center !important; gap: 16px !important; }
          .reg-sidebar-steps { display: flex !important; flex-direction: row !important; gap: 12px !important; margin-top: 0 !important; }
          .reg-main { padding: 24px 16px !important; align-items: flex-start !important; }
          .reg-card { padding: 24px 20px !important; }
          .reg-role-grid { grid-template-columns: 1fr 1fr !important; }
          .reg-brand { margin-bottom: 0 !important; }
        }
      `}</style>
      <div className="reg-container" style={{ display: 'flex', minHeight: '100vh', fontFamily: 'sans-serif' }}>
        <div className="reg-sidebar" style={{ width: '280px', background: '#0F2744', padding: '40px 24px', display: 'flex', flexDirection: 'column' }}>
          <div className="reg-brand" style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '48px' }}>
            <div style={{ width: '36px', height: '36px', background: '#4fa3e0', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '13px', color: 'white', flexShrink: 0 }}>FPI</div>
            <div>
          ...
          <div className="reg-sidebar-steps" style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
        ...
        <div className="reg-main" style={{ flex: 1, background: '#f4f6f9', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px' }}>
          <div className="reg-card" style={{ width: '100%', maxWidth: '480px', background: 'white', borderRadius: '12px', padding: '40px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
            ...
            {step === 2 && (
              <>
                ...
                <div className="reg-role-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                  {[
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
                  <select value={form.currency} onChange={e => update('currency', e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', color: '#1a1a2e', background: 'white', width: '80px' }}>
                    <option>£</option><option>$</option><option>€</option><option>₹</option><option>¥</option><option>A$</option><option>C$</option>
                  </select>
                  <select value={form.revenue} onChange={e => update('revenue', e.target.value)} style={{ flex: 1, padding: '10px 14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', color: '#1a1a2e', background: 'white' }}>
                    ...
                    <option>Under 1M</option><option>1M – 10M</option><option>10M – 100M</option><option>100M – 1B</option><option>Over 1B</option>
                  </select>
                </div>
                {message && <p style={{ color: 'red', fontSize: '13px', marginBottom: '12px' }}>{message}</p>}
                <div style={{ display: 'flex', gap: '12px' }}>
